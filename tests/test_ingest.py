...
def test_unsafe_official_url_is_dropped_with_a_warning():
    """A remote page's <a href> is attacker-controlled text. A javascript:
    href would otherwise become fallback_url and then every round's url,
    so drop it (falling back to source_url) and say so in the preview."""
    html = """
    <html><body><article>
      <h1 class="gh-article-title">Some Concert</h1>
    ...
    """
    source = "https://ramen.events/some-concert/"
    parsed = parse_ramen_event(html, source)
    assert any("unsafe" in w.lower() for w in parsed.warnings)
    assert "javascript" not in "".join(parsed.warnings).lower()


def test_safe_official_url_is_kept():
    html = """
    <html><body><article>
      <h1 class="gh-article-title">Some Concert</h1>
      <section class="gh-content">
        <p>Official website: <a href="https://example.com/e">here</a></p>
    ...
    """
    parsed = parse_ramen_event(html, "https://ramen.events/some-concert/")
    assert parsed.rounds[0].url == "https://example.com/e"
