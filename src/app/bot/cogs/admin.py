...
class Admin(commands.Cog):
    ...
    async def promote_editor(
        ...
        await interaction.response.send_message(
            f"**{member.name}** is now an editor.", ephemeral=True
        )
    ...
    async def demote_editor(self, interaction: discord.Interaction, member: discord.Member) -> None:
        ...
        if settings.is_editor(member.id):
            await interaction.response.send_message(
                f"**{member.name}** is env-managed (EDITOR_WHITELIST) — edit `.env` instead.",
                ephemeral=True,
            )
            return
        async with SessionMaker() as session:
            ...
            await session.commit()
        await interaction.response.send_message(
            f"**{member.name}** is no longer an editor.", ephemeral=True
        )
    ...
    async def list_editors_cmd(self, interaction: discord.Interaction) -> None:
        ...
        if not editors:
            await interaction.response.send_message("No editors yet.", ephemeral=True)
            return
        lines = [
            f"**{e['username'] or e['id']}** ({e['id']}){' — env' if e['env'] else ''}"
            for e in editors
        ]
