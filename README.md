...
- [x] A restore drill that proves the app *runs* — counting rows in a gunzipped dump proves the file parses, which is not the question a backup exists to answer; the drill now compares the backup's `alembic_version` against `alembic heads` (so you know whether `upgrade head` comes first) and ends by booting the app web-only against the restored copy until real concerts render
- [x] Agent read API — `/api/v1` serves the catalogue, the tag vocabulary, the discovery-lead queue and pending drafts over bearer-token HTTP, GET only, so an agent loop no longer runs through your clipboard
- [x] AI triage calibrated against real data — the first live press of the pipeline against the real catalogue (95 pages fetched, 511 open leads; three models given byte-identical prompts, with the export's own rounds and leg counts as the answer key) and the nine fixes it produced, including a classify pass that was unbounded and failed on a real queue by size rather than judgement
- [x] Round watch — the catalogue's quiet ladders: tracked concerts holding no future deadline, surfaced for review instead of going stale unnoticed (the cheapest of three shapes filed under "round watch"; the per-concert re-fetch shape is the round poll below, and a discovery-matcher round-gap dimension remains on the list)
- [x] Outcome correction — a lottery result recorded by mistake can be taken back; the result buttons only opened from *Applied*, so a round reading Won or Lost had no way back, and a mis-pressed "Not applying" silently ended the campaign
...
- [x] Round poll, both phases — a flag-gated daily pass (`ROUND_POLL_ENABLED`, off by default) re-reads each quiet concert's own official page with DeepSeek and records what its ladder appears to have grown as reviewable proposals; an admin now reviews each one on its own draft page at `/admin/quiet-ladders/proposals/{event_id}`, every field pre-filled with the model's value and its quoted source line beside it, and can approve it onto the concert as a real round (queue-synced in the same transaction) or dismiss it for good — completing the per-concert re-fetch shape filed under "round watch" (a discovery-matcher round-gap dimension remains on the list)
- [x] Reminder offsets down to the minute — the second box of a reminder sentence takes an exact `h:mm` (`0:05`, `1:30`) instead of a whole-hours dropdown, on both the preset editor and the concert page; `/remindme` takes a `minutes_before`; and the welcome wizard's fine-tune list gains *5 minutes* and *30 minutes*, which is what a first-come-first-served sale actually wants reminding about. It also fixes what your rules had been telling you: a "3 hours before" rule described itself as *Same day* on the concert page and `same-day` in Discord, because both surfaces read the day count alone and ignored the hours entirely
