...
> ## ⚠ SCOPE NOTICE (added 2026-07-29, at `a62c6a8`)
>
> This document is a SNAPSHOT of `a9b98a0` and has not been rewritten. What it
> describes is still accurate; what it OMITS is now substantial, and its own
> premise -- "everything you need without re-reading the tree" -- is what makes
> that dangerous. Eleven builds have merged since. Nothing below mentions any
> of the following, so treat this file as complete only for what predates
> 2026-07-27, and read `CLAUDE.md` (kept current) for the rest:
>
> - **Per-leg outcome truth** — `RoundOutcomeDay`, `record_round_day_result` /
>   `record_remaining_days_lost`, `secured_day_ids_by_round`, migration
>   `7deda13c049b`. A round covering Sat+Sun can be won on one leg and lost on
>   the other, and NO rows means all. This changes invariant 2, which §3 states
>   in its pre-2026-07-27 form.
> - **Dead-concert rules** — `all_legs_cancelled` (the Python twin of
>   `discoverable_concert_criterion`), and the ruling that a tag attached to a
>   dead concert notifies nobody and applies no preset.
> - **`handle_newly_tagged` timing** — it must be called only once the legs are
>   written. `create_concert_row` RETURNS its newly attached tags instead of
>   consuming them (its §8 signature `-> Concert` is wrong; it is now
>   `-> tuple[Concert, list[Tag]]`), and `generate_event_id` treats a reserved
>   id as taken.
> - **Home/concert-page shape** — `domain/board.py:visible_rungs`, the
>   per-concert "Coming up" blocks, the per-leg settled-round folds, and
>   `service.performer_clusters`.
> - **The operations trio** — `delivery_log` + `/admin/deliveries` + the
>   per-tick digest, the held-and-cancellable `/admin/broadcast`
>   (`Notification.send_after_utc`), and the flag-gated `/admin/rehearsal`
>   (`routes/rehearsal.py`, registered only when `settings.rehearsal_enabled`).
>   Migrations `4d208d277be7` and `aebefef6ca70`.
> - **Error pages** — real 403/404/422/500 (`error.html`), HTML for a
>   navigation and the original JSON for an XHR.
>
> Kept as a snapshot rather than rewritten because that is what it is; it is
> also untracked on purpose (commit `788aa52`), so it is a local handoff aid,
> not a repo document.
