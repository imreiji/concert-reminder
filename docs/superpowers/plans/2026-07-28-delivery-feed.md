...
- After `alembic revision --autogenerate`, ALWAYS hand-edit the revision: replace `app.db.models.UTCDateTime()` with `sa.DateTime()` and delete the `import app.db.models` line.
...
- Create: `migrations/versions/<rev>_add_delivery_log.py` (autogenerated, then hand-edited)
...
```bash
git add src/app/db/models.py migrations/versions/ tests/test_delivery_log.py
