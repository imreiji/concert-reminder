...
**PROJECT-SPECIFIC — READ THIS FIRST.** In this repository `CLAUDE.md` is deliberately only a SUMMARY (~100 lines). The enforceable rules live in `.claude/skills/dekimasen/references/*.md` — `invariants.md`, `architecture.md`, `migrations.md`, `i18n.md`, `ui-conventions.md`, `testing.md`, `workflows.md` — and `AGENTS.md` carries the routing table saying which covers what. Checking a change against `CLAUDE.md` alone will report CLEAN while that change violates an invariant spelled out only in a reference. So: read `CLAUDE.md` for the map, then read every reference matching the areas the change touches, and treat those references as binding project instructions exactly as you would CLAUDE.md itself. When you quote a violated guideline, cite the reference file and rule you are quoting.

Your primary responsibilities:

1. **Analyze Recent Changes**: Focus on the most recent code additions, modifications, or file creations. You should identify what has changed by examining the current state against the expected behavior defined in CLAUDE.md and the skill references it delegates to.
